  ...
  if (!$request[0]) {
    ...
    foreach (getPGTableRange($postgreSQL, $schemaName,
      ...
      if ($month != $date[1]) {
        ...
        drawMonth($postgreSQL, $year, $month, $schemaName, 'existsPGTable', '/nonDNSTraffic');
        echo '</div>';
      }
    }
  }
    ...
    $date = $request[0];
    $title = 'Protocol Violations for ' . $date;
    $logo = '<table width="100%">' .
              '<tr>' .
                '<td class="sub_title" width="15%" align="left">' .
                  getPreviousPGDay($postgreSQL, $schemaName, $date, '/nonDNSTraffic') .
               '</td>' .
              '<td width="60%" align="center">' .
                $title .
              '</td>' .
              '<td class="sub_title" width="15%" align="right">' .
                getNextPGDay($postgreSQL, $schemaName, $date, '/nonDNSTraffic') .
              '</td>' .
            '</tr>' .
          '</table>';
    ...
    if ($request[1]) {
      ...
      $title .= ' (' . $request[1] . ')';
      $logo = makeLogo($postgreSQL, $schemaName, $date, $title, '/nonDNSTraffic', '/' . $internalIP);
    }
    else {
      exit;
    }
    ...
    if (!$request[2]) {
      ...
      $result = pg_query($postgreSQL, 'SELECT "externalIP", SUM("numBytes"), SUM("numPackets"), "asNumber", "countryNumber", COUNT(*) ' .
                                      ...
                                      'GROUP BY "externalIP", "asNumber", "countryNumber" ORDER BY COUNT(*) DESC');
      while ($row = pg_fetch_row($result)) {
        $rowNumber % 2 ? $rowClass = 'odd' : $rowClass = 'even';
        echo '<tr class="' . $rowClass . '">' .
               '<td class="center">';
                 if ($infectedIPs && array_search($row[0], $infectedIPs) !== false) {
                   echo '<a class="text" href="/nonDNSTraffic/' . $date . '/' . $internalIP . '/' . long2ip($row[0]) . '">' .
                          '<font color="#FF0000">' .
                            long2ip($row[0]) .
                          '</font>' .
                        '</a>';
      ...
      $title = 'Protocol Violations for ' . $date . ' (' . $internalIP . '/' . $externalIP . ')';
      $logo = 'Protocol Violations for ' . $date . ' (' . $internalIP . '/' . styleIP($infectedIPs, $numericExternalIP);
      $logo = makeLogo($postgreSQL, $schemaName, $date,
                       'Protoco Violations for ' . $date . ' (' . $internalIP . '/' . styleIP($infectedIPs, $numericExternalIP) . ')',
                       '/nonDNSTraffic', '/' . $internalIP . '/' . $externalIP);
      include('include/header.php');
      ...
      echo '<div class="table">' .
             '<table width="100%" cellspacing="1">' .
               '<tr>' .
                 ...
                 '</td>' .
               '</tr>';
      $result = pg_query($postgreSQL, 'SELECT "protocol", "internalPort", "externalPort", "numBytes", "numPackets", "minPacketSize", "maxPacketSize", ' .
      ...
      while ($row = pg_fetch_assoc($result)) {
        ...
        echo '<tr class="' . $rowClass . '">' .
             ...
             '</td>' . 
             '<td class="center">' .
               getServiceName($row['protocol'], $row['internalPort'], $row['externalPort'], INTERNAL_INITIATOR) .
             '</td>' .
             '<td class="center">' .
