  ...
  if (!$request[0]) {
    ...
    foreach (getPGTableRange($postgreSQL, $schemaName,
      ...
      if ($month != $date[1]) {
        ...
        drawMonth($postgreSQL, $year, $month, $schemaName, 'existsPGTable', '/encryptedP2PTraffic');
        echo '</div>';
      }
    }
  }
    ...
    $date = $request[0];
    $title = 'Encrypted P2P Traffic for ' . $date;
    $logo = '<table width="100%">' .
              '<tr>' .
                '<td class="sub_title" width="15%" align="left">' .
                  getPreviousPGDay($postgreSQL, $schemaName, $date, '/encryptedP2PTraffic') .
               '</td>' .
              '<td width="60%" align="center">' .
                $title .
              '</td>' .
              '<td class="sub_title" width="15%" align="right">' .
                getNextPGDay($postgreSQL, $schemaName, $date, '/encryptedP2PTraffic') .
              '</td>' .
            '</tr>' .
          '</table>';
    ...
    if ($request[1]) {
      ...
      $title .= ' (' . $request[1] . ')';
      $logo = makeLogo($postgreSQL, $schemaName, $date, $title, '/encryptedP2PTraffic', '/' . $internalIP);
    }
    else {
      exit;
    }
    ...
    if (!$request[2]) {
      ...
      $result = pg_query($postgreSQL, 'SELECT "externalIP", SUM("numBytes"), SUM("numPackets"), "asNumber", "countryNumber", COUNT(*) ' .
                                      ...
                                      'AND "type" = ' . ENCRYPTED_TYPE . ' GROUP BY "externalIP", "asNumber", "countryNumber" ORDER BY SUM("numPackets") DESC');
      while ($row = pg_fetch_row($result)) {
        $rowNumber % 2 ? $rowClass = 'odd' : $rowClass = 'even';
        echo '<tr class="' . $rowClass . '">' .
               '<td class="center">';
                 if ($infectedIPs && array_search($row[0], $infectedIPs) !== false) {
                   echo '<a class="text" href="/encryptedP2PTraffic/' . $date . '/' . $internalIP . '/' . long2ip($row[0]) . '">' .
                          '<font color="#FF0000">' .
                            long2ip($row[0]) .
                          '</font>' .
                        '</a>';
      ...
      $title = 'Encrypted P2P Traffic for ' . $date . ' (' . $internalIP . '/' . $externalIP . ')';
      $logo = 'Encrypted P2P Traffic for ' . $date . ' (' . $internalIP . '/' . styleIP($infectedIPs, $numericExternalIP);
      $logo = makeLogo($postgreSQL, $schemaName, $date,
                       'Encrypted P2P Traffic for ' . $date . ' (' . $internalIP . '/' . styleIP($infectedIPs, $numericExternalIP) . ')',
                       '/EncryptedP2PTraffic', '/' . $internalIP . '/' . $externalIP);
      include('include/header.php');
      ...
      $result = pg_query($postgreSQL, 'SELECT "internalPort", "externalPort", "numBytes", "numPackets", "minPacketSize", "maxPacketSize", "numBytes", ' .
                                      ...
                                      '\' AND "externalIP" = \'' . $numericExternalIP . '\' AND "type" = ' . ENCRYPTED_TYPE . ' ORDER BY "numPackets" DESC');
