#ifndef INCLUDED_PMULTIMAP_HPP
#define INCLUDED_PMULTIMAP_HPP
...
namespace shared{
	...
	/// persistent and can be used by two or more processes for reading. pmultimap
	...
	/// searched.
	///
	/// \remark pmultimap currently doesn't support iteration over the entire map. The
	...
	///
	/// \remark Though it is named pmultimap it is actually a multimap. It is a
	/// plural map in the sense each pmultimap is represented by many segment_size
	...
	class pmultimap{
		...
		typedef boost::interprocess::multimap<_Key, _Data, _Compare, allocator_type> container_type;

		typedef typename container_type::iterator iterator;
		typedef typename container_type::const_iterator const_iterator;
		typedef std::pair<typename container_type::iterator, typename container_type::iterator> range;
		typedef std::pair<typename container_type::const_iterator, typename container_type::const_iterator> const_range;
		typedef size_t size_type;

		/// Default constructor
		pmultimap():segment(NULL), p_allocator(NULL), p_map_container(NULL), segment_size(0), current_file_number(0), read_prev_end(false){
		}
		...
		/// \param (segment_size) Size of the memory resident portion of pmultimap.
		...
		bool create(const char* file_name, size_type segment_size){
			try{
				...
				p_map_container = segment->construct<container_type>("ARLMultiMap")(_Compare(), *p_allocator);
		...
		/// \brief Extends an existing pmultimap instance by appending new maps.
		/// \param (file_name) Path of the file which has the pmultimap to extend.
		/// \param (segment_size) Size of memory resident portion of pmultimap.
		/// \return Returns true if the pmultimap is sucessfully loaded for
		/// extending; False otherwise.
		///
		/// Given an existing pmultimap the function loads the pmultimap from disk to
		/// append/change the pmultimap. The function never creates a new pmultimap if
		...
		bool extend(const char* file_name, size_t segment_size){
			try{
				...
				p_map_container = (segment->find<container_type>("ARLMultiMap")).first;
				if(p_map_container == NULL){
					return false;
				}
			}catch(...){
		...
		iterator insert(const value_type& v){
			iterator status (p_map_container->end());

			try{
				iterator status = p_map_container->insert(v);
			}catch(...){
				...
				if(segment->get_size() < segment_size){	
					...
					if(!_create_new_segment()){
						...
						return status; // can't do much if this fails.
					}
				}

				// Try insertion again into the new segment.
				try{
					status = p_map_container->insert(v);
				}catch(...){
		...
		const_iterator find(const key_type& key) const{
			return p_map_container->find(key);
                ...
                /*const_range equal_range(const key_type& key) const {
                    return p_map_container->equal_range(key);
                }*/

                range equal_range(const key_type& key) const {
                    return p_map_container->equal_range(key);
                }
		...
		/// \brief The function returns the encapsulated container.
		///
		/// \todo This is a bad idea!
		///
		container_type* get_container(){ return p_map_container;}
		const container_type* get_container() const{ return p_map_container;}
		...
		private:
			...
			bool _create_new_segment(){
				...
				try{
					...
					_p_map_container = _segment->construct<container_type>("ARLMultiMap")(_Compare(), *_p_allocator);
			...
			bool _load_next_segment(){
				...
				try{
					...
					_p_map_container = (_segment->find<container_type>("ARLMultiMap")).first;
					if(_p_map_container == NULL){
						--current_file_number; 
						delete _p_allocator;
						delete _segment;
