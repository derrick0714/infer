...
$role_short = array_pop(explode('/', $argv[0]));
...
*/
	...
	$query =
		"SELECT r.ip as internal_ip, " .
			  'r."numHosts" as num_hosts, ' .
			  'r."numBytes" as num_bytes, ' .
			  'r."startTime" as start_time, ' .
			  'r."endTime" as end_time ' .
		"FROM \"Roles\".\"" . date('Y-m-d', $date) . "\" AS r, " .
			"\"Maps\".role_names as rn " .
		"WHERE (r.role = rn.role_id) " .
			"AND (rn.role_short = '" . $role_short . "') " .
