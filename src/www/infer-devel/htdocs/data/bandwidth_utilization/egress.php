...
$ht = @pg_query($pg, "SELECT \"interval_start\", \"egress_bytes_per_second\" FROM \"Stats\".\"".($ip ? "host_" : "")."bandwidth_utilization\" WHERE ".($ip ? "\"host\" = '$ip' AND " : "")."\"interval_start\" >= '".$time_begin."' AND \"interval_start\" < '".$time_end."' ORDER BY \"interval_start\" ASC");
...
$egress_bandwidth_utilization = array(
...
echo json_encode($egress_bandwidth_utilization);
?>