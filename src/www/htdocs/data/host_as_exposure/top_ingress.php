...
$ht = @pg_query($pg, "SELECT \"as\".\"asn\", \"asName\", \"asDescription\", \"as\".\"ingress_bytes\", \"as\".\"egress_bytes\" FROM \"Maps\".\"asNames\" INNER JOIN (SELECT * FROM \"HostASExposure\".\"".date('Y-m-d', $date)."\" WHERE \"asn\" != '0' AND \"host\" = '$host' ORDER BY \"ingress_bytes\" DESC LIMIT 10) AS \"as\" ON \"asNumber\" = \"as\".\"asn\"");
