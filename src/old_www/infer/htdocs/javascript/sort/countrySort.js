<!--
function countrySort(tableID, columnID, elementIdentifier)
{ 
	if(!overThousandRecords)//confirm that the user want to proceed
	{
	   countrSort(tableID, columnID, elementIdentifier)
	   return;
	}
	 ...
	 if(confirm("This operation may take up to one minute. Do you want to proceed?"))
	 {
        ...
        setTimeout("countrSort('"+tableID+"', '"+columnID+"', '"+elementIdentifier+"')", 4000);	    
     } 
	 else
	 {
        return;
...
function countrSort(tableID, columnID, elementIdentifier)
{
  ...
  function getA(index)
  {
	 ...
	 var strip_start = value.lastIndexOf('title')+7;
