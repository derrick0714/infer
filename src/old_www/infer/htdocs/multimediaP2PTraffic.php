  ...
  if (!$request[0]) {
    ...
    foreach (getPGTableRange($postgreSQL, $schemaName,
      ...
      if ($month != $date[1]) {
        ...
        drawMonth($postgreSQL, $year, $month, $schemaName, 'existsPGTable', '/multimediaP2PTraffic');
        echo '</div>';
      }
    }
  }
    ...
    $date = $request[0];
    $title = 'Multimedia P2P Traffic for ' . $date;
    $logo = '<table width="100%">' .
              '<tr>' .
                '<td class="sub_title" width="15%" align="left">' .
                  getPreviousPGDay($postgreSQL, $schemaName, $date, '/multimediaP2PTraffic') .
               '</td>' .
              '<td width="60%" align="center">' .
                $title .
              '</td>' .
              '<td class="sub_title" width="15%" align="right">' .
                getNextPGDay($postgreSQL, $schemaName, $date, '/multimediaP2PTraffic') .
              '</td>' .
            '</tr>' .
          '</table>';
    ...
    if ($request[1]) {
      ...
      $title .= ' (' . $request[1] . ')';
      $logo = makeLogo($postgreSQL, $schemaName, $date, $title, '/multimediaP2PTraffic', '/' . $internalIP);
    }
    else {
      exit;
    }
    ...
    if (!$request[2]) {
      ...
      $result = pg_query($postgreSQL, 'SELECT "externalIP", SUM("numBytes"), SUM("numPackets"), "asNumber", "countryNumber", COUNT(*) ' .
                                      ...
                                      'AND "type" = ' . MULTIMEDIA_TYPE . ' GROUP BY "externalIP", "asNumber", "countryNumber" ORDER BY SUM("numPackets") DESC');
      while ($row = pg_fetch_row($result)) {
        $rowNumber % 2 ? $rowClass = 'odd' : $rowClass = 'even';
        echo '<tr class="' . $rowClass . '">' .
               '<td class="center">';
                 if ($infectedIPs && array_search($row[0], $infectedIPs) !== false) {
                   echo '<a class="text" href="/multimediaP2PTraffic/' . $date . '/' . $internalIP . '/' . long2ip($row[0]) . '">' .
                          '<font color="#FF0000">' .
                            long2ip($row[0]) .
                          '</font>' .
                        '</a>';
      ...
      $title = 'Multimedia P2P Traffic for ' . $date . ' (' . $internalIP . '/' . $externalIP . ')';
      $logo = 'Multimedia P2P Traffic for ' . $date . ' (' . $internalIP . '/' . styleIP($infectedIPs, $numericExternalIP);
      $logo = makeLogo($postgreSQL, $schemaName, $date,
                       'Multimedia P2P Traffic for ' . $date . ' (' . $internalIP . '/' . styleIP($infectedIPs, $numericExternalIP) . ')',
                       '/multimediaP2PTraffic', '/' . $internalIP . '/' . $externalIP);
      include('include/header.php');
      ...
      $result = pg_query($postgreSQL, 'SELECT "internalPort", "externalPort", "numBytes", "numPackets", "minPacketSize", "maxPacketSize", "numBytes", ' .
                                      ...
                                      '\' AND "externalIP" = \'' . $numericExternalIP . '\' AND "type" = ' . MULTIMEDIA_TYPE . ' ORDER BY "numPackets" DESC');
