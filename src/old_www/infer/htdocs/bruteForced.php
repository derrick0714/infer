  ...
  if (!$request[0]) {
    ...
    foreach (getPGTableRange($postgreSQL, $schemaName,
      ...
      if ($month != $date[1]) {
        ...
        drawMonth($postgreSQL, $year, $month, $schemaName, 'existsPGTable', '/bruteForced');
        echo '</div>';
      }
    }
  }
    ...
    $date = $request[0];
    $title = 'Brute Forced Hosts for ' . $date;
    $logo = '<table width="100%">' .
              '<tr>' .
                '<td class="sub_title" width="15%" align="left">' .
                  getPreviousPGDay($postgreSQL, $schemaName, $date, '/bruteForced') .
               '</td>' .
              '<td width="60%" align="center">' .
                $title .
              '</td>' .
              '<td class="sub_title" width="15%" align="right">' .
                getNextPGDay($postgreSQL, $schemaName, $date, '/bruteForced') .
              '</td>' .
            '</tr>' .
          '</table>';
    ...
    if (!$request[1]) {
      $logo = makeLogo($postgreSQL, $schemaName, $date, $title, '/bruteForced', '/' . $internalIP);
      include('include/header.php');
      echo '<div class="table">' .
             '<table width="100%" cellspacing="1">' .
               '<tr>' .
                 ...
                 '</td>' .
               '</tr>';
      $result = pg_query($postgreSQL, 'SELECT "destinationIP", COUNT(DISTINCT "sourceIP") FROM "' . $schemaName . '"."' . $date .
                                      '" GROUP BY "destinationIP" ORDER BY COUNT(DISTINCT "sourceIP") DESC');
      while ($row = pg_fetch_assoc($result)) {
        $rowNumber % 2 ? $rowClass = 'odd' : $rowClass = 'even';
        echo '<tr class="' . $rowClass . '">' .
               '<td class="center">' .
                 '<a class="text" href="/bruteForced/' . $date . '/' . long2ip($row['destinationIP']) . '">' .
      ...
      if (!$request[2]) {
        ...
        $result = pg_query($postgreSQL, 'SELECT "destinationPort", COUNT(*) FROM "' . $schemaName . '"."' . $date .
                                        '" WHERE "destinationIP" = \'' . $numericInternalIP . '\' ' .
                                        'GROUP BY "destinationPort" ORDER BY COUNT(*) DESC');
        while ($row = pg_fetch_assoc($result)) {
          $rowNumber % 2 ? $rowClass = 'odd' : $rowClass = 'even';
          echo '<tr class="' . $rowClass . '">' .
                 '<td class="center">' .
                   '<a class="text" href="/bruteForced/' . $date . '/' . $internalIP . '/' . $row['destinationPort'] . '">' .
        ...
        include('include/services.php');
        $title = 'Brute Forced Hosts for ' . $date . ' (' . $internalIP . '/' . getServiceName(6, 0, $destinationPort, INTERNAL_INITIATOR) . ')';
        include('include/header.php');
        echo '<div class="table">' .
               '<table width="100%" cellspacing="1">' .
                 '<tr>' .
                   ...
                   '</td>' .
                 '</tr>';
        $result = pg_query($postgreSQL, 'SELECT "sourceIP", "startTime", "endTime", "numAttempts" FROM "' .
                                        $schemaName . '"."' . $date . '" WHERE "destinationIP" = \'' . $numericInternalIP .
