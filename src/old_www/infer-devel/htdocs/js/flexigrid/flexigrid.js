...
(function($){
						...
						function ()
							...
							.click(
								function (e) 
									{ 
										...
										if (p.onClickRow) p.onClickRow(e, $(this));
									}
							)
							.mousedown(
								function (e)
							...
							.hover(
								...
								{ 
									...
									if (p.onHoverRow) {
										if (p.onHoverRow[0])
											p.onHoverRow[0](e, $(this));
									}
								},
								function () {
									if (p.onHoverRow) {
										if (p.onHoverRow[1])
											p.onHoverRow[1](e, $(this));
									}
								}
							)
							;
	...
	$.fn.flexSwitchCol = function(cdrag,cdrop) { // function to switch columns

		return this.each( function() {
				if (this.grid) {
					this.grid.switchCol(cdrag,cdrop);
					
					if (cdrag>cdrop)
						$('th:eq('+cdrop+')',this.grid.hDiv).before($('th:eq('+cdrag+')',this.grid.hDiv));
					else
						$('th:eq('+cdrop+')',this.grid.hDiv).after($('th:eq('+cdrag+')',this.grid.hDiv));
				}
			});
