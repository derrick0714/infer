...
tab_uri_notify = function(e) {
	...
	// TODO check for subsection and handle that somehow...
	var section = 'index';
	if (argv[3] != undefined) {
		section = argv[3];
	}
	...
	$.getJSON('/data/date_checker/' + argv[2], function(content) {
		...
		$('#previous_date').text(function()
		{
			...
			$(this).parent().attr('href', '#/' + argv[0] + '/' + argv[1] + '/' + temp_date + '/' + argv.slice(3).join('/'));
			return temp_date;
		});
		...
		$.getJSON('/data/host_role/' + section + '/' + ip + '/' + current_date +
					'/' + argv.slice(4).join('/') + '?page=0&rp=0',
			function(content)
		{
			...
			{
				var breadcrumb = [];
				breadcrumb.push('<a href="#/' + argv.slice(0, 3).join('/') + '">Indicators - ' + argv.slice(1, 3).join(' - ') + '</a>');
				...
				$('#flex1').flexigrid({
					...
					url: '/data/host_role/' + section + '/' + ip + '/' + current_date + '/' + argv.slice(4).join('/'),
					...
					onClickRow: function(e, r) {
						if ($(r).attr('id').substr(4, 3) == 'url') {
							...
							var tmp_rp = $.address.parameter('rp');
							$.address.queryString('');
							if (tmp_rp) {
								$.address.parameter('rp', tmp_rp);
							}
...
<div id="inner_wrapper">
	<div class="navigation">
		...
		<div id="status_title">Indicators for <span id="current_host"></span> on <span id="current_date"></span></div>
