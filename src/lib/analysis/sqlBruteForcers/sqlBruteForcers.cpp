...
extern "C" {
  ...
  bool initialize(SharedState &sharedState, ModuleState &moduleState) {
	...
	if (moduleState.conf.get(attemptLimit,
			...
			!= configuration::OK)
	{
		cerr << "analysis_sqlBruteForcers: missing or invalid attempt-limit"
			 << endl;
		return false;
	}
	...
	if (moduleState.conf.get(minFanin,
			...
			!= configuration::OK)
	{
		cerr << "analysis_sqlBruteForcers: missing or invalid min-fan-in"
			 << endl;
		return false;
	}
	...
	if (moduleState.conf.get(minFanout,
			...
			!= configuration::OK)
	{
		cerr << "analysis_sqlBruteForcers: missing or invalid min-fan-out"
			 << endl;
		return false;
	}
	...
	if (moduleState.conf.get(asn_white_list,
			...
			!= configuration::OK)
	{
		cerr << "analysis_sqlBruteForcers: missing or invalid asn-white-list"
			 << endl;
		return false;
	}
 ...
 void aggregate(const FlowStats *flowStats, size_t) { 
    //filtering down to TCP connections on SQL port
    if( isTCP(flowStats) && (isSQLPort(flowStats->destinationPort()) || isSQLPort(flowStats->sourcePort())) ){
	...
	if( (typeInit == EXTERNAL_INITIATOR && isSQLPort(hostPair.internalPort)) || 
            (typeInit == INTERNAL_INITIATOR && asnWhitelist.find(asn) == asnWhitelist.end() && isSQLPort(hostPair.externalPort))){
