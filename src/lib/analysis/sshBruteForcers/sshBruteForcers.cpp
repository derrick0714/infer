...
extern "C" {
  ...
  bool initialize(SharedState &sharedState, ModuleState &moduleState) {
	...
	if (moduleState.conf.get(attemptLimit,
			...
			!= configuration::OK)
	{
		cerr << "analysis_sshBruteForcers: missing or invalid attempt-limit"
			 << endl;
		return false;
	}
	...
	if (moduleState.conf.get(minFanin,
			...
			!= configuration::OK)
	{
		cerr << "analysis_sshBruteForcers: missing or invalid min-fan-in"
			 << endl;
		return false;
	}
	...
	if (moduleState.conf.get(minFanout,
			...
			!= configuration::OK)
	{
		cerr << "analysis_sshBruteForcers: missing or invalid min-fan-out"
			 << endl;
		return false;
	}
	...
	if (moduleState.conf.get(asn_white_list,
			...
			!= configuration::OK)
	{
		cerr << "analysis_sshBruteForcers: missing or invalid asn-white-list"
			 << endl;
		return false;
	}
 ...
 void aggregate(const FlowStats *flowStats, size_t) { 
    //filtering down to TCP connections on SSH port
    if( isTCP(flowStats) && (isSSHPort(flowStats->destinationPort()) || isSSHPort(flowStats->sourcePort())) ){
	...
	if( (typeInit == EXTERNAL_INITIATOR && isSSHPort(hostPair.internalPort)) || 
            (typeInit == INTERNAL_INITIATOR && asnWhitelist.find(asn) == asnWhitelist.end() && isSSHPort(hostPair.externalPort))){
