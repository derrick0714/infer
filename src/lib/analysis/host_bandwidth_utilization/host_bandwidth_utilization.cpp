...
#include "host_bandwidth_utilization_datum.hpp"
...
// stats collection
// host -> (ingress, egress) -> interval_start -> bw
typedef map<uint32_t, pair<map<uint32_t, double>, map<uint32_t, double> > >
	host_bw_map_t;
host_bw_map_t _bw_utilization;

// configuration options
size_t _interval_length;
size_t _buffer_count;
string _stats_schema;
string _host_bandwidth_utilization_stats_table;
...
extern "C" {
	bool initialize(SharedState &sharedState, ModuleState &moduleState) {
		...
		if (moduleState.conf.get(_interval_length,
				...
				!= configuration::OK)
		{
			cerr << "analysis_host_bandwidth_utilization: "
		...
		if (moduleState.conf.get(_host_bandwidth_utilization_stats_table,
								 "host-bandwidth-utilization-stats-table",
								 "analysis_host_bandwidth_utilization")
				!= configuration::OK)
		{
			cerr << "analysis_host_bandwidth_utilization: "
					"missing or invalid host-bandwidth-utilization-stats-table"
				 << endl;
			return false;
		}
	...
	void aggregate(FlowStats *flowStats, size_t &) {
					   ...
					   *(_shared_state_p->localNetworks)))
		{
			...
			bw = &(_bw_utilization[flowStats->sourceIP()].second);
		}
		else {
			if (!isInternal(flowStats->destinationIP(),
							*(_shared_state_p->localNetworks)) ||
			...
			bw = &(_bw_utilization[flowStats->destinationIP()].first);
		}
			   ...
			   const char *date)
	{
		...
		if (strptime(date, "%Y-%m-%d", &tm_) == NULL) {
			cerr << "analysis_host_bandwidth_utilization: invalid date: "
				 << date << endl;
			return -1;
		}
		...
		string query("DELETE FROM \"" + _stats_schema + "\".\"" +
					 _host_bandwidth_utilization_stats_table +
		...
		PostgreSQLBulkWriter<host_bandwidth_utilization_datum>
			writer(pg_conn,
				   _stats_schema,
				   _host_bandwidth_utilization_stats_table);

		host_bandwidth_utilization_datum datum;
		int rows;
		while (_bw_utilization.begin() != _bw_utilization.end()) {
			host_bw_map_t::iterator host(_bw_utilization.begin());
			for (uint32_t i(begin); i < end; i += _interval_length) {
				datum.host = host->first;
				datum.time_seconds = i;
				datum.ingress_bytes_per_second =
					host->second.first[i] / _interval_length;
				datum.egress_bytes_per_second =
					host->second.second[i] / _interval_length;
