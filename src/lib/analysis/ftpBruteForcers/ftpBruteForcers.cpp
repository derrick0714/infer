...
extern "C" {
  ...
  bool initialize(SharedState &sharedState, ModuleState &moduleState) {
	...
	if (moduleState.conf.get(attemptLimit,
			...
			!= configuration::OK)
	{
		cerr << "analysis_ftpBruteForcers: missing or invalid attempt-limit"
			 << endl;
		return false;
	}
	...
	if (moduleState.conf.get(minFanin,
			...
			!= configuration::OK)
	{
		cerr << "analysis_ftpBruteForcers: missing or invalid min-fan-in"
			 << endl;
		return false;
	}
	...
	if (moduleState.conf.get(minFanout,
			...
			!= configuration::OK)
	{
		cerr << "analysis_ftpBruteForcers: missing or invalid min-fan-out"
			 << endl;
		return false;
	}
	...
	if (moduleState.conf.get(asn_white_list,
			...
			!= configuration::OK)
	{
		cerr << "analysis_ftpBruteForcers: missing or invalid asn-white-list"
			 << endl;
		return false;
	}
 ...
 void aggregate(const FlowStats *flowStats, size_t) { 
    //filtering down to TCP connections on FTP port
    if( isTCP(flowStats) && (isFTPPort(flowStats->destinationPort()) || isFTPPort(flowStats->sourcePort())) ){
	...
	if( (typeInit == EXTERNAL_INITIATOR && isFTPPort(hostPair.internalPort)) || 
            (typeInit == INTERNAL_INITIATOR && asnWhitelist.find(asn) == asnWhitelist.end() && isFTPPort(hostPair.externalPort))){
 ...
 int commit(PostgreSQLConnection &pg_conn, size_t &flushSize, const char *date) {
        ...
        if (!preparePGTable(pg_conn.connection(), BRUTE_FORCERS_SCHEMA_NAME, date,
                            BRUTE_FORCERS_TABLE_SCHEMA, true)){
              return -1;
        }
