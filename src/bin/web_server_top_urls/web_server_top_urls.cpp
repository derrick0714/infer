...
int main(int argc, char **argv) {
	...
	if (conf.get(data_directory, "data-directory", "web_server_top_urls", true)
			!= configuration::OK)
	{
	...
	string web_server_top_urls_schema;
	if (conf.get(web_server_top_urls_schema, "web-server-top-urls-schema", "web_server_top_urls")
			!= configuration::OK)
	{
		cerr << argv[0] << ": web-server-top-urls-schema required" << endl;
		return 1;
	}

	size_t top_url_count;
	if (conf.get(top_url_count, "top-url-count", "web_server_top_urls")
			!= configuration::OK)
	{
		cerr << argv[0] << ": top-url-count required" << endl;
		return 1;
	}

	vector<string> exclude_suffixes;
	if (conf.get(exclude_suffixes, "exclude-suffix", "web_server_top_urls")
			== configuration::BAD_DATA)
	{
		cerr << argv[0] << ": invalid exclude-suffix" << endl;
		return 1;
	}
	...
	// stats structures
	typedef map<uint32_t, map<string, url_stat_t> > url_stats_t;
	url_stats_t url_stats;
	...
	while ((reader_status = request_reader.read(http_request)) == E_SUCCESS) {
		...
		url = http_request.host() + http_request.uri();
		for (vector<string>::const_iterator ex_suffix(exclude_suffixes.begin());
			 ex_suffix != exclude_suffixes.end();
			 ++ex_suffix)
		{
			size_t pos = url.find('?');
			if (ex_suffix->length() > pos || ex_suffix->length() > url.length()) {
				continue;
			}
			if (pos == string::npos) {
				suffix = url.substr(url.length() - ex_suffix->length());
			}
			else {
				suffix = url.substr(pos - ex_suffix->length(), ex_suffix->length());
				/*
				cout << "suffix:    " << suffix << endl;
				cout << "ex_suffix: " << *ex_suffix << endl;
				*/
			}
			transform(suffix.begin(),
					  suffix.end(),
					  suffix.begin(),
					  (int (*)(int)) tolower);
			if (suffix == *ex_suffix) {
		...
		++url_stats[server_ip][url].request_count;
		if (http_request.time() <
				url_stats[server_ip][url].first_request_time)
		{
			url_stats[server_ip][url].first_request_time =
				http_request.time();
		}
		if (http_request.time() >
				url_stats[server_ip][url].last_request_time)
		{
			url_stats[server_ip][url].last_request_time =
				http_request.time();
		}
	...
	PostgreSQLBulkWriter<top_url_datum> writer(pg_conn,
											  web_server_top_urls_schema,
											  date,
											  true);
	...
	top_url_datum top_url;
	set<top_url_datum> top_urls;
	for (url_stats_t::iterator i(url_stats.begin());
		 i != url_stats.end();
		 ++i)
	{
		server_ip = i->first;
		for (map<string, url_stat_t>::iterator j(i->second.begin());
			 j != i->second.end();
			 ++j)
		{
			top_url.server_ip = server_ip;
			top_url.url = j->first;
			top_url.request_count = j->second.request_count;
			top_url.first_request_time = j->second.first_request_time;
			top_url.last_request_time = j->second.last_request_time;
			top_urls.insert(top_url);

			if (top_urls.size() > top_url_count) {
				top_urls.erase(top_urls.begin());
			}
		}

		// insert the top N urls for this IP
		for (set<top_url_datum>::const_iterator k(top_urls.begin());
			 k != top_urls.end();
			 ++k)
		{
			writer.write(*k);
