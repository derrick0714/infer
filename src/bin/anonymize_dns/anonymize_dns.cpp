...
int main(int argc, char **argv) {
	...
	if (conf.get(data_directory, "data-directory", "anonymize_dns", true)
			!= configuration::OK)
	{
	...
	if (conf.get(output_directory, "output-directory", "anonymize_dns")
			!= configuration::OK)
	{
	...
	IPv4Network ip_xor;
	if (conf.get(ip_xor, "ip-xor", "anonymize_dns") != configuration::OK) {
	...
	if (conf.get(internal_networks,
				 "internal-network",
				 "anonymize_dns") != configuration::OK)
	{
	...
	boost::shared_ptr<StrftimeWriteEnumerator<DNS, fixed_time_functor> >
		output_enumerator(new StrftimeWriteEnumerator<DNS,
													  fixed_time_functor>(
			output_directory, "%Y/%m/%d/dns_%H", start_time_functor));

	InferFileWriter<FlatFileWriter<DNS>,
					StrftimeWriteEnumerator<DNS,
											fixed_time_functor> >
	...
	while ((error_status = reader.read(dns)) == E_SUCCESS) {
		...
		{
			if (!src_xor && 
					net->rawIsInNetwork(dns.rawClientIP()))
			{
				dns.rawClientIP(dns.rawClientIP() ^ ip_xor.rawNetwork());
				src_xor = true;
			}
			if (!dst_xor && 
					net->rawIsInNetwork(dns.rawServerIP()))
			{
				dns.rawServerIP(
					dns.rawServerIP() ^ ip_xor.rawNetwork());
				dst_xor = true;
			}
		}

		// write the anonymized record
		output_writer.write(&dns);
	}
