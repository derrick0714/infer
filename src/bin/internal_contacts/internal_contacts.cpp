...
int main(int argc, char *argv[]) {
	...
	if (conf.get(host_index_schema, "host-index-schema", "internal_contacts")
			!= configuration::OK)
	{
	...
	string stats_schema;
	if (conf.get(stats_schema, "stats-schema", "internal_contacts")
			!= configuration::OK)
	{
	...
	uint32_t id, host;
	size_t external_contact_count;
	for (size_t i(0); i < row_count; ++i) {
		id = lexical_cast<uint32_t>(PQgetvalue(result, i, 0));
		host = lexical_cast<uint32_t>(PQgetvalue(result, i, 1));

		external_contact_count = out_degree(vertex(id, network_graph),
											network_graph);

		if (decimal) {
			cout << host << '\t' << external_contact_count << endl;
		}
		else {
			cout << ip::address_v4(host) << '\t' << external_contact_count
				 << endl;
		}
	}
	PQclear(result);
