...
int main(int argc, char **argv) {
	...
	if (conf.get(data_directory, "data-directory", "anonymize_http_response", true)
			!= configuration::OK)
	{
	...
	if (conf.get(output_directory, "output-directory", "anonymize_http_response")
			!= configuration::OK)
	{
	...
	IPv4Network ip_xor;
	if (conf.get(ip_xor, "ip-xor", "anonymize_http_response") != configuration::OK) {
	...
	if (conf.get(internal_networks,
				 "internal-network",
				 "anonymize_http_response") != configuration::OK)
	{
	...
	boost::shared_ptr<StrftimeWriteEnumerator<HTTPResponse, fixed_time_functor> >
		output_enumerator(new StrftimeWriteEnumerator<HTTPResponse,
													  fixed_time_functor>(
			output_directory, "%Y/%m/%d/http_response_%H", start_time_functor));

	InferFileWriter<FlatFileWriter<HTTPResponse>,
					StrftimeWriteEnumerator<HTTPResponse,
											fixed_time_functor> >
	...
	rot13_5 encrypt;
	while ((error_status = reader.read(response)) == E_SUCCESS) {
		...
		{
			if (!src_xor && 
					net->rawIsInNetwork(response.raw_source_ip()))
			{
				response.raw_source_ip(response.raw_source_ip() ^ ip_xor.rawNetwork());
				src_xor = true;
			}
			if (!dst_xor && 
					net->rawIsInNetwork(response.raw_destination_ip()))
			{
				response.raw_destination_ip(
					response.raw_destination_ip() ^ ip_xor.rawNetwork());
				dst_xor = true;
			}
		}

		// write the anonymized record
		output_writer.write(&response);
	}
