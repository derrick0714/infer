...
int main(int argc, char *argv[]) {
	...
	if (conf.get(host_index_schema, "host-index-schema", "mutual_contacts")
			!= configuration::OK)
	{
	...
	if (conf.get(host_index_pair_schema, "host-index-pair-schema", "mutual_contacts")
			!= configuration::OK)
	{
	...
	string stats_schema;
	if (conf.get(stats_schema, "stats-schema", "mutual_contacts")
			!= configuration::OK)
	{
	...
	if (conf.get(network_stats_table, "network-stats-table", "mutual_contacts")
			!= configuration::OK)
	{
	...
	if (conf.get(privacy_threshold, "privacy-threshold", "mutual_contacts")
			!= configuration::OK)
	{
	...
	if (conf.get(dye_iterations, "dye-iterations", "mutual_contacts")
			!= configuration::OK)
	{
	...
	uint32_t host_index;
	if (host == 0) {
		host_index = numeric_limits<uint32_t>::max();
	}
	else {
		host_index = get_index_from_host(host,
										 host_index_schema,
										 date,
										 pg_conn);
		if (host_index == numeric_limits<uint32_t>::max()) {
	...
	if (host != 0 && host_index >= internal_ip_count)
	{
	...
	for (uint32_t id(index_begin); id < index_end; ++id) {
		...
		for (size_t i(0); i < internal_ip_count; ++i) {
			...
			/*
				results.insert(make_pair(d(i), get_host_from_index(i,
														   host_index_schema,
														   date,
														   pg_conn)));
			*/
...
/*
bool get_external_contacts(unordered_set<uint32_t> &external_ips,
						   uint32_t ip,
						   PostgreSQLConnection &pg_conn,
						   const string &date)
{
	string query("SELECT \"externalIPs\" FROM \"HostPairs\".\"" + date +
				 "\" WHERE \"internalIP\" = '" + lexical_cast<string>(ip) +
				 "';");
	cerr << "EXT: " << query << endl;
	...
	for (size_t i(0); i < row_count; ++i) {
		string ip_string(PQgetvalue(result, i, 0));
		vector<string> ip_strings;
		explodeString(ip_strings,
					  ip_string.substr(1, ip_string.length() - 2),
					  ",");
		for (vector<string>::const_iterator str(ip_strings.begin());
			 str != ip_strings.end();
			 ++str)
		{
			external_ips.insert(lexical_cast<uint32_t>(*str));
		}
	}
	PQclear(result);
...
bool get_internal_contacts(unordered_set<uint32_t> &internal_ips,
						   uint32_t ip,
						   PostgreSQLConnection &pg_conn,
						   const string &date)
{
	string query("SELECT \"internalIP\" FROM \"HostPairs\".\"" + date +
				 "\" WHERE '" + lexical_cast<string>(ip) +
				 "' = ANY (\"externalIPs\");");
	cerr << "INT: " << query << endl;
	//cerr << "DEBUG: query: " << query << endl;
	PGresult *result(PQexec(pg_conn.connection(), query.c_str()));
	...
	for (size_t i(0); i < row_count; ++i) {
		...
		for (vector<string>::const_iterator str(ip_strings.begin());
			 ...
			 ++str)
		{
			internal_ips.insert(lexical_cast<uint32_t>(*str));
		}
	}
	PQclear(result);
...
bool populate_privacy_graph(boost::numeric::ublas::matrix<double>
							 ...
							 size_t privacy_threshold)
{
	unordered_set<uint32_t> external_ips;
	unordered_set<uint32_t> internal_ips;
	if (!get_external_contacts(external_ips, ip, pg_conn, date)) {
		return false;
	}
	for (unordered_set<uint32_t>::const_iterator external_ip(
			external_ips.begin());
		 external_ip != external_ips.end();
		 ++external_ip)
	{
		if (external_ip_set.find(*external_ip) != external_ip_set.end()) {
			// if we've recursed into this ip already, skip it
			continue;
		}
		internal_ips.clear();
		if (!get_internal_contacts(internal_ips, *external_ip, pg_conn, date))
		{
			return false;
		}
		if (internal_ips.size() > privacy_threshold) {
			continue;
		}
		external_ip_set.insert(*external_ip);
		for (unordered_set<uint32_t>::const_iterator i(internal_ips.begin());
			i != internal_ips.end();
			++i)
		{
			if (internal_ip_index.find(*i) == internal_ip_index.end()) {
				internal_ip_index.insert(make_pair(*i,
												   internal_ip_index.size()));
				privacy_graph.resize(internal_ip_index.size(),
									  internal_ip_index.size());
			}
			unordered_set<uint32_t>::const_iterator j(i);
			for (++j;
				 j != internal_ips.end();
				 ++j)
			{
				if (internal_ip_index.find(*j) == internal_ip_index.end()) {
					internal_ip_index.insert(
						make_pair(*j, internal_ip_index.size()));
					privacy_graph.resize(internal_ip_index.size(),
										  internal_ip_index.size());
				}
				++(privacy_graph(internal_ip_index[*i],
								  internal_ip_index[*j]));
				++(privacy_graph(internal_ip_index[*j],
								  internal_ip_index[*i]));
			}
			if (!populate_privacy_graph(privacy_graph,
										 ip,
