...
int main(int argc, char **argv) {
	...
	if (conf.get(data_directory, "data-directory", "anonymize_hbf", true)
			!= configuration::OK)
	{
	...
	if (conf.get(output_directory, "output-directory", "anonymize_hbf")
			!= configuration::OK)
	{
	...
	IPv4Network ip_xor;
	if (conf.get(ip_xor, "ip-xor", "anonymize_hbf") != configuration::OK) {
	...
	if (conf.get(internal_networks,
				 "internal-network",
				 "anonymize_hbf") != configuration::OK)
	{
	...
	boost::shared_ptr<StrftimeWriteEnumerator<ZlibCompressedHBF, fixed_time_functor> >
		output_enumerator(new StrftimeWriteEnumerator<ZlibCompressedHBF,
													  fixed_time_functor>(
			output_directory, "%Y/%m/%d/hbf_%H", start_time_functor));

	InferFileWriter<FlatFileWriter<ZlibCompressedHBF>,
					StrftimeWriteEnumerator<ZlibCompressedHBF,
											fixed_time_functor> >
	...
	while ((error_status = reader.read(zhbf)) == E_SUCCESS) {
		...
		{
			if (!src_xor && 
					net->rawIsInNetwork(zhbf.rawSourceIP()))
			{
				zhbf.rawSourceIP(zhbf.rawSourceIP() ^ ip_xor.rawNetwork());
				src_xor = true;
			}
			if (!dst_xor && 
					net->rawIsInNetwork(zhbf.rawDestinationIP()))
			{
				zhbf.rawDestinationIP(
					zhbf.rawDestinationIP() ^ ip_xor.rawNetwork());
				dst_xor = true;
			}
		}

		// write the anonymized record
		output_writer.write(&zhbf);
	}
