...
int main(int argc, char *argv[]) {
	...
	if (conf.get(host_index_schema, "host-index-schema", "related_hosts")
			!= configuration::OK)
	{
	...
	if (conf.get(host_index_pair_schema, "host-index-pair-schema", "related_hosts")
			!= configuration::OK)
	{
	...
	string stats_schema;
	if (conf.get(stats_schema, "stats-schema", "related_hosts")
			!= configuration::OK)
	{
	...
	if (conf.get(network_stats_table, "network-stats-table", "related_hosts")
			!= configuration::OK)
	{
	...
	if (conf.get(privacy_threshold, "privacy-threshold", "related_hosts")
			!= configuration::OK)
	{
	...
	if (conf.get(dye_iterations, "dye-iterations", "related_hosts")
			!= configuration::OK)
	{
	...
	boost::numeric::ublas::matrix<double> orig_privacy_graph(privacy_graph);
	...
	for (uint32_t id(index_begin); id < index_end; ++id) {
			 ...
			 ++i)
		{
			...
			if (decimal) {
				...
				cout << host << '\t' << tmp_ip << '\t' << i->first << '\t' <<  orig_privacy_graph(id, i->second) << endl;
			}
			else {
				//cout << ip::address_v4(host) << '\t' << ip::address_v4(i->second) << '\t' << i->first << endl;
				cout << ip::address_v4(host) << '\t' << ip::address_v4(tmp_ip) << '\t' << i->first << '\t' <<  orig_privacy_graph(id, i->second) << endl;
			}
		}
	}
