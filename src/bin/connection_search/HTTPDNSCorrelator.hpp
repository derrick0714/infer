...
int HTTPDNSCorrelator<ReaderType, WriterType, ResponseWriterType>::run() {
		 ...
		 ++i)
	{
		...
		*/
		if (i->request().type() == 'q') {
			tmpRawSourceIP = i->request().rawSourceIP();
			tmpRawDestinationIP = i->request().rawDestinationIP();
		}
		else {
			tmpRawSourceIP = i->response().rawSourceIP();
			tmpRawDestinationIP = i->response().rawDestinationIP();
		}
