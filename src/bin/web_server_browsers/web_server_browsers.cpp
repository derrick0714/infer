...
struct web_server_browser_stat_t {
	web_server_browser_stat_t()
		:request_count(0),
...
int main(int argc, char **argv) {
	...
	if (conf.get(data_directory, "data-directory", "web_server_web_server_browsers", true)
			!= configuration::OK)
	{
	...
	string web_server_browsers_schema;
	if (conf.get(web_server_browsers_schema, "web-server-browsers-schema", "web_server_browsers")
			!= configuration::OK)
	{
		cerr << argv[0] << ": web-server-browsers-schema required" << endl;
		return 1;
	}
	...
	if (conf.get(local_networks_string,
				 "local-networks",
				 "web_server_web_server_browsers",
				 true) !=
			configuration::OK)
	{
	...
	// stats structures
	typedef map<uint32_t, map<pair<string, string>, web_server_browser_stat_t> >
		web_server_browser_stats_t;
	web_server_browser_stats_t web_server_browser_stats;
	...
	while ((reader_status = request_reader.read(http_request)) == E_SUCCESS) {
		...
		{
			browser.first = "Unknown";
			browser.second = "";
		}

		++web_server_browser_stats[server_ip][browser].request_count;
		if (http_request.time() <
				web_server_browser_stats[server_ip][browser].first_request_time)
		{
			web_server_browser_stats[server_ip][browser].first_request_time =
				http_request.time();
		}
		if (http_request.time() >
				web_server_browser_stats[server_ip][browser].last_request_time)
		{
			web_server_browser_stats[server_ip][browser].last_request_time =
				http_request.time();
		}
	...
	PostgreSQLBulkWriter<web_server_browser_datum> writer(pg_conn,
											  web_server_browsers_schema,
											  date,
											  true);
	...
	web_server_browser_datum web_server_browser;
	for (web_server_browser_stats_t::iterator i(web_server_browser_stats.begin());
		 i != web_server_browser_stats.end();
		 ++i)
	{
		server_ip = i->first;
		for (map<pair<string, string>, web_server_browser_stat_t>::iterator j(i->second.begin());
			 j != i->second.end();
			 ++j)
		{
			web_server_browser.server_ip = server_ip;
			web_server_browser.browser = j->first;
			web_server_browser.request_count = j->second.request_count;
			web_server_browser.first_request_time = j->second.first_request_time;
			web_server_browser.last_request_time = j->second.last_request_time;
			writer.write(web_server_browser);
		}
	}
