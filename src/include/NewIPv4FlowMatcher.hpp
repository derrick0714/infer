#ifndef INFER_INCLUDE_NEWIPV4FLOWMATCHER_HPP_
#define INFER_INCLUDE_NEWIPV4FLOWMATCHER_HPP_
...
class NewIPv4FlowMatcher {
  public:
	...
	bool isMatch(const T &flow) const {
		...
		if (!_sourceNetworks.empty()) {
			...
			for (NetworksContainer::const_iterator
				 ...
				 ++it)
			{
				if (it->rawIsInNetwork(flow.raw_source_ip())) {
					match = true;
					break;
				}
			}
		...
		if (!_destinationNetworks.empty()) {
			...
			for (NetworksContainer::const_iterator
				 ...
				 ++it)
			{
				if (it->rawIsInNetwork(flow.raw_destination_ip())) {
					match = true;
					break;
				}
			}
		...
		if (!_eitherNetworks.empty()) {
			...
			{
				if (it->rawIsInNetwork(flow.raw_source_ip()) ||
					it->rawIsInNetwork(flow.raw_destination_ip())) 
				{
					match = true;
					break;
				}
		...
		// check source_port against ranges
		if (!_sourcePortRanges.empty()) {
			...
			for (PortRangesContainer::const_iterator
				 ...
				 ++it)
			{
				if (flow.source_port() >= it->first &&
					flow.source_port() <= it->second)
				{
					match = true;
					break;
				}
		...
		// check destination_port against ranges
		if (!_destinationPortRanges.empty()) {
			...
			for (PortRangesContainer::const_iterator
				 ...
				 ++it)
			{
				if (flow.destination_port() >= it->first &&
					flow.destination_port() <= it->second)
				{
					match = true;
					break;
				}
	...
	NewIPv4FlowMatcher reverse() const {
		return NewIPv4FlowMatcher(_protocolRanges,
							   _destinationNetworks,
							   _sourceNetworks,
							   _eitherNetworks,
							   _destinationPortRanges,
  ...
  private:
	...
	NewIPv4FlowMatcher(const ProtocolRangesContainer &protocolRanges,
