...
class PostgreSQLConnection {
  public:
	...
	PostgreSQLConnection(const PostgreSQLConnection &c)
		:_connection(NULL),
		 _host(c._host),
		 _hostaddr(c._hostaddr),
		 _port(c._port),
		 _dbname(c._dbname),
		 _user(c._user),
		 _password(c._password),
		 _connect_timeout(c._connect_timeout),
		 _options(c._options),
		 _sslmode(c._sslmode),
		 _requiressl(c._requiressl),
		 _krbsrvname(c._krbsrvname),
		 _gsslib(c._gsslib),
		 _service(c._service),
		 _error(false),
		 _errorMsg()
