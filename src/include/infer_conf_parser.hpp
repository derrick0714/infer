...
// ----------------------------------------------------------------------------
#ifndef BOOST_PROPERTY_TREE_INFER_CONF_PARSER_HPP_INCLUDED
#define BOOST_PROPERTY_TREE_INFER_CONF_PARSER_HPP_INCLUDED
...
namespace boost { namespace property_tree { namespace infer_conf_parser
{

    /**
     * Determines whether the @c flags are valid for use with the infer_conf_parser.
     * @param flags value to check for validity as flags to infer_conf_parser.
    ...
    class infer_conf_parser_error: public file_parser_error
    {
    public:
        /**
         * Construct an @c infer_conf_parser_error
         ...
         *             encountered.
         */
        infer_conf_parser_error(const std::string &message,
     ...
     * @throw infer_conf_parser_error If a format violation is found.
     * @param stream Stream from which to read in the property tree.
     * @param[out] pt The property tree to populate.
     */
    template<class Ptree>
    void read_infer_conf(std::basic_istream<
                  ...
                  Ptree &pt)
    {
        ...
        while (stream.good())
        {
            ...
            if (!stream.good() && !stream.eof())
                BOOST_PROPERTY_TREE_THROW(infer_conf_parser_error(
                    "read error", "", line_no));
     ...
     * @throw infer_conf_parser_error In case of error deserializing the property tree.
     * @param filename Name of file from which to read in the property tree.
     * @param[out] pt The property tree to populate.
     * @param loc The locale to use when reading in the file contents.
     */
    template<class Ptree>
    void read_infer_conf(const std::string &filename, 
                  Ptree &pt,
     ...
     * @throw infer_conf_parser_error In case of error translating the property tree to
     ...
     */
    template<class Ptree>
    void write_infer_conf(std::basic_ostream<
     ...
     */
    template<class Ptree>
    void write_infer_conf(const std::string &filename,
                   const Ptree &pt,
                   int flags = 0,
                   const std::locale &loc = std::locale())
    {
        ...
        stream.imbue(loc);
        try {
            write_infer_conf(stream, pt, flags);
        }
        catch (infer_conf_parser_error &e) {
            BOOST_PROPERTY_TREE_THROW(infer_conf_parser_error(
                e.message(), filename, e.line()));
...
{
    using infer_conf_parser::infer_conf_parser_error;
    using infer_conf_parser::read_infer_conf;
    using infer_conf_parser::write_infer_conf;
} }
